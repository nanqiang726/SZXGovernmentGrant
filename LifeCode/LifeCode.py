        ...
        if name and ischinese is True :
            ...
            if birthday and len(birthday)==10:
                ...
                if operator.eq('Y',str(yn)) or operator.eq('y',str(yn)):
                    ...
                    print('你的主性格数字：%s，在五行中属：%s'%(O,self.wuxing(O)))
                    print('你的幸运数字：%s' % luckyNum)
                    print('你的天赋数字：%s' % talentNum)
                    print('你的生命数字：%s'%self.addRule(talentNum,0))
                    print('你的潜意识密码：%s%s%s' % (O,I,L))
                    print('你的内心密码：%s%s%s' % (O,M,N))
                    print('你的外心密码：%s%s%s' % (S,R,T))
                    print('你的外在动向等于：%s,你是【%s】'%(outTrend,outTrendType))
